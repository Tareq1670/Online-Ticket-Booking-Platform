...
const TimeBox = ({ value, label, colorScheme = "blue" }) => {
    ...
    return (
        <div className="flex flex-col items-center">
            <div
                className={`${c.box} border rounded-lg px-2 py-1 min-w-[38px] text-center shadow-sm overflow-hidden`}
            >
            ...
            <span
                className={`text-[8px] ${c.label} uppercase tracking-wider mt-1 font-bold`}
            >
                {label}
            </span>
        </div>
...
const generatePremiumTicketPDF = (booking, transaction, user) => {
    ...
    const fullTxnId = String(transactionId).toUpperCase();

    const departureDate = new Date(booking.departureDate);
    const paidDate = transaction?.paymentDate
    ...
    const timeStr = departureDate.toLocaleTimeString("en-US", {
        hour: "2-digit",
        minute: "2-digit",
    });
    ...
    const passengerName = (user?.name || "Valued Passenger").toUpperCase();
    const passengerEmail = user?.email || "N/A";
    ...
    doc.setFontSize(6.5);
    doc.setFont("helvetica", "bold");
    doc.text("PASSENGER NAME", M, 49);

    doc.setTextColor(255, 255, 255);
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    const maxNameWidth = W / 2 - M - 5;
    doc.text(truncateText(doc, passengerName, maxNameWidth), M, 55);

    doc.setTextColor(167, 243, 208);
    doc.setFontSize(6.5);
    doc.setFont("helvetica", "bold");
    doc.text("EMAIL", W / 2 + 5, 49);

    doc.setTextColor(255, 255, 255);
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    const maxEmailWidth = W - M - (W / 2 + 5);
    doc.text(truncateText(doc, passengerEmail, maxEmailWidth), W / 2 + 5, 55);
    ...
    // ============ ROUTE CARD (68-143mm) ============
    doc.setFillColor(255, 255, 255);
    doc.roundedRect(M, 68, W - M * 2, 75, 6, 6, "F");
    doc.setDrawColor(209, 250, 229);
    doc.setLineWidth(0.5);
    doc.roundedRect(M, 68, W - M * 2, 75, 6, 6, "S");

    doc.setTextColor(107, 114, 128);
    doc.setFontSize(7);
    doc.setFont("helvetica", "bold");
    doc.text("DEPARTURE", M + 10, 80);
    doc.text("ARRIVAL", W - M - 10, 80, { align: "right" });

    const cityMaxWidth = (W - M * 2) / 2 - 35;

    doc.setTextColor(6, 78, 59);
    doc.setFontSize(20);
    doc.setFont("helvetica", "bold");
    let fromCity = (booking.from || "—").toUpperCase();
    while (doc.getTextWidth(fromCity) > cityMaxWidth && fromCity.length > 3) {
        fromCity = fromCity.slice(0, -1);
    }
    if (fromCity !== (booking.from || "—").toUpperCase()) fromCity += "..";
    doc.text(fromCity, M + 10, 94);

    let toCity = (booking.to || "—").toUpperCase();
    while (doc.getTextWidth(toCity) > cityMaxWidth && toCity.length > 3) {
        toCity = toCity.slice(0, -1);
    }
    if (toCity !== (booking.to || "—").toUpperCase()) toCity += "..";
    doc.text(toCity, W - M - 10, 94, { align: "right" });

    const fromWidth = doc.getTextWidth(fromCity);
    const toWidth = doc.getTextWidth(toCity);
    const arrowY = 90;
    const arrowX1 = M + 10 + fromWidth + 8;
    const arrowX2 = W - M - 10 - toWidth - 8;

    if (arrowX2 > arrowX1 + 10) {
    ...
    doc.roundedRect(M + 10, 105, W - M * 2 - 20, 12, 3, 3, "F");
    doc.setDrawColor(209, 250, 229);
    doc.setLineWidth(0.3);
    doc.roundedRect(M + 10, 105, W - M * 2 - 20, 12, 3, 3, "S");

    doc.setTextColor(6, 78, 59);
    doc.setFontSize(8);
    doc.setFont("helvetica", "bold");
    doc.text(transport, W / 2, 112.5, { align: "center" });

    const ticketTitle = (booking.title || "Travel Ticket").toUpperCase();
    doc.setTextColor(17, 24, 39);
    doc.setFontSize(10);
    doc.setFont("helvetica", "bold");
    const maxTitleWidth = W - M * 2 - 20;
    doc.text(truncateText(doc, ticketTitle, maxTitleWidth), W / 2, 130, {
        align: "center",
    });
    ...
    if (booking.vendorName) {
        ...
        doc.setFontSize(7);
        doc.setFont("helvetica", "normal");
        doc.text(`Operated by: ${booking.vendorName}`, W / 2, 137, {
            align: "center",
        });
    }

    // ============ JOURNEY DETAILS CARD (148-203mm) ============
    doc.setFillColor(255, 255, 255);
    doc.roundedRect(M, 148, W - M * 2, 55, 6, 6, "F");
    doc.setDrawColor(209, 250, 229);
    doc.setLineWidth(0.5);
    doc.roundedRect(M, 148, W - M * 2, 55, 6, 6, "S");

    doc.setTextColor(6, 78, 59);
    doc.setFontSize(8);
    doc.setFont("helvetica", "bold");
    doc.text("JOURNEY DETAILS", M + 10, 158);

    doc.setDrawColor(16, 185, 129);
    doc.setLineWidth(0.3);
    doc.line(M + 10, 161, W - M - 10, 161);

    const detailY = 170;
    const col1 = M + 10;
    const col2 = W / 2 + 5;
    const colMaxWidth = (W - M * 2) / 2 - 15;
    ...
    const details = [
        ...
        {
            label: "SEATS",
            value: `${booking.quantity || 1} Seat(s)`,
            x: col1,
            y: detailY + 18,
        },
    ...
    details.forEach((d) => {
        ...
        doc.setFontSize(10);
        doc.setFont("helvetica", "bold");
        doc.text(truncateText(doc, d.value, colMaxWidth), d.x, d.y + 7);
    });

    doc.setDrawColor(209, 250, 229);
    doc.setLineWidth(0.2);
    doc.line(W / 2, 163, W / 2, 200);

    // ============ TOTAL PAID (208-238mm) ============
    doc.setFillColor(6, 78, 59);
    doc.roundedRect(M, 208, W - M * 2, 30, 6, 6, "F");

    doc.setTextColor(167, 243, 208);
    doc.setFontSize(7);
    doc.setFont("helvetica", "bold");
    doc.text("TOTAL AMOUNT PAID", M + 12, 219);

    doc.setTextColor(255, 255, 255);
    doc.setFontSize(18);
    doc.setFont("helvetica", "bold");
    doc.text(
        `BDT ${transaction?.amount || booking.totalPrice || 0}`,
        M + 12,
        230,
    );
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
        `${booking.quantity || 1} x BDT ${booking.unitPrice || 0} per seat`,
        W - M - 12,
        224,
        { align: "right" },
    );
    doc.text(`Paid: ${paidDateStr}`, W - M - 12, 230, { align: "right" });
    ...
    // ============ PAYMENT RECEIPT (249-287mm) - 38mm tall ============
    doc.setFillColor(255, 255, 255);
    doc.roundedRect(M, 249, W - M * 2, 38, 6, 6, "F");
    doc.setDrawColor(209, 250, 229);
    doc.setLineWidth(0.5);
    doc.roundedRect(M, 249, W - M * 2, 38, 6, 6, "S");

    doc.setTextColor(6, 78, 59);
    doc.setFontSize(8);
    doc.setFont("helvetica", "bold");
    doc.text("PAYMENT RECEIPT", M + 8, 257);

    doc.setDrawColor(16, 185, 129);
    doc.setLineWidth(0.3);
    doc.line(M + 8, 259, W - M - 8, 259);

    const receiptColW = (W - M * 2 - 20) / 2;
    const leftColX = M + 8;
    const rightColX = M + 8 + receiptColW + 4;

    // Booking Reference - Left
    doc.setTextColor(107, 114, 128);
    doc.setFontSize(6);
    doc.setFont("helvetica", "bold");
    doc.text("BOOKING REFERENCE", leftColX, 266);

    doc.setFillColor(240, 253, 244);
    doc.roundedRect(leftColX, 268, receiptColW, 9, 2, 2, "F");
    doc.setDrawColor(167, 243, 208);
    doc.setLineWidth(0.3);
    doc.roundedRect(leftColX, 268, receiptColW, 9, 2, 2, "S");
    doc.setTextColor(6, 78, 59);
    doc.setFontSize(9);
    doc.setFont("courier", "bold");
    doc.text(shortId, leftColX + 3, 274);

    // Transaction ID - Right
    doc.setTextColor(107, 114, 128);
    doc.setFontSize(6);
    doc.setFont("helvetica", "bold");
    doc.text("TRANSACTION ID", rightColX, 266);

    doc.setFillColor(240, 253, 244);
    doc.roundedRect(rightColX, 268, receiptColW, 9, 2, 2, "F");
    doc.setDrawColor(167, 243, 208);
    doc.setLineWidth(0.3);
    doc.roundedRect(rightColX, 268, receiptColW, 9, 2, 2, "S");
    doc.setTextColor(6, 78, 59);
    doc.setFont("courier", "bold");

    // Auto-fit transaction ID
    let txnFontSize = 9;
    doc.setFontSize(txnFontSize);
    while (doc.getTextWidth(fullTxnId) > receiptColW - 6 && txnFontSize > 5) {
        txnFontSize -= 0.5;
        doc.setFontSize(txnFontSize);
    }
    doc.text(fullTxnId, rightColX + 3, 274);

    // Notice line at bottom of receipt card
    doc.setTextColor(107, 114, 128);
    doc.setFontSize(5.5);
    doc.setFont("helvetica", "italic");
    doc.text(
        "Present this ticket at boarding  |  Valid for one journey only  |  Non-transferable",
        W / 2,
        283,
        { align: "center" },
    );

    // ============ FOOTER (290-294mm) - SMALL ============
    doc.setFillColor(6, 78, 59);
    ...
    doc.setFontSize(5.5);
    doc.setFont("helvetica", "bold");
    doc.text("TICKETIX", M, 294);

    doc.setFont("helvetica", "normal");
    doc.text("www.ticketix.com  |  support@ticketix.com", W / 2, 294, {
        align: "center",
    });

    doc.setFontSize(5);
    doc.text(`Generated: ${new Date().toLocaleString()}`, W - M, 294, {
        align: "right",
    });
...
const BookingCard = ({
    ...
    const relatedTransaction = transactions?.find(
        (t) => getPlainId(t.bookingId) === bookingId,
    );
                ...
                <div className="mt-auto flex flex-col gap-3">
                    <div className="bg-gradient-to-br from-emerald-50 to-green-50 dark:from-emerald-950/40 dark:to-green-950/40 rounded-xl px-4 py-3 border-2 border-emerald-200 dark:border-emerald-900 text-center">
                        ...
                        <p className="text-2xl font-black text-emerald-700 dark:text-emerald-400 mt-1">
                            ৳{relatedTransaction?.amount || booking.totalPrice}
                        </p>
                        ...
                        >
                            <div className="relative flex flex-col items-center px-5 pb-7 pt-6 sm:px-7 sm:pb-7 sm:pt-7">
                                <Button
                                    variant="none"
                                    onClick={
                                        !isProcessing ? onClose : undefined
                                    }
                        ...
                        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                            ...
                            >
                                <motion.div
                                    ...
                                    className="cursor-pointer rounded-2xl bg-white/95 p-4 text-center shadow-lg dark:bg-[#07111F]/90"
                                >
